...
> **Hammer-lanche**（山札上6枚を捨て、捨てた基本水エネルギー1枚につき100ダメージ）を主軸に、**大量の基本水エネルギー**で火力を確保。  
> **Kyogre** で捨て札の水を再利用しエネ切れを防ぐ。  
> **カルミネ / リーリエ / ラムダ / レシーバー** でドロー・サーチし、手札事故を減らす。
...
|--------|-------|------|
| デフォルト | 10000 | 強く優先 |
| Ultra Ball | 4000 または **-1** | 手札水≥3、または手札≥4かつキーカード不足時のみ |
| Carmine | 3000 または **-1** | 場ユキカブリ＋手札メガが揃っていると不使用 |
| Lillie | 3100 または **-1** | Carmine より僅かに優先。進化待ちなら不使用 |
| Petrel (1219) | 3050 または **-1** | 山札からサポーターサーチ。進化待ちなら不使用 |
| Receiver (1134) | 3200 / 3000 / **-1** | 捨て札にサポーターがあれば高優先。早期展開時は中優先 |
| Precious Trolley (1126) | 10000 | 専用分岐なし（汎用 PLAY） |
